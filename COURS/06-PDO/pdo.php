...
<body>
    ...
    try {
        $pdo = new PDO(
            'mysql:host=127.0.0.1;port=3308;charset=utf8;dbname=societe',  // driver mysql + nom du serveur de la BDD + nom de la BDD 
    ...
    // $resultat = $pdo->exec("INSERT INTO employes (prenom, nom, sexe, service, date_embauche, salaire) VALUES ('saliha', 'lili', 'f', 'informatique', '2024-02-08', 5000)");
    ...
    $result = $pdo->query("SELECT * FROM employes WHERE prenom = 'saliha'");
    ...
    */
    ...
    // bindValue() reçoit une variable OU une valeur directement. Le marqueur point uniquement vers la valeur : si celle-ci change, il faudra refaire un bindValue lors d'un nouvel execute() pour tenir compte de cette nouvelle valeur (sinon le marqueur conserve l'ancienne valeur).
    // bindValue() reçoit une variable OU une valeur directement. Le marqueur point uniquement vers la valeur : si celle-ci change, il faudra refaire un bindValue lors d'un nouvel execute() pour tenir compte de cette nouvelle valeur (sinon le marqueur conserve l'ancienne valeur).
    ...
    $resultat->execute(['lili', 'saliha']); // on peut remplacer les 2 bindValue et le execute() précédents par cette ligne, en passant un array à la méthode execute(). Les valeurs sont dans le même ordre que les marqueurs dans la requête 
    ...
    $resultat->execute([':nom' => 'lili', ':prenom' => 'saliha']); // on associe les marqueurs à leur valeur directement avec des crochets
