...
if (isset($_GET['article']) && isset($_GET['couleur']) && isset($_GET['prix']) && !empty($_GET['article']) && !empty($_GET['couleur']) && !empty($_GET['prix'])) {
...
if (isset($_GET['prix']) && !empty($_GET['couleur'])) {
    $miniPrix = 20;

    if ($_GET['prix'] < $miniPrix) {
        echo '<p> le prix est insuffisamment bas ! </p>';
    } else {
        echo '<p> Le prix est suffisamment haut ! </p>';
    }
}
